...
from keras.layers import Convolution2D, Flatten, Dense, Lambda, Reshape
...
TRAIN_INTERVAL = 4  # The agent selects 4 actions between successive update
...
LOAD_NETWORK = False
TRAIN = True
FOLDER_TAG = ENV_NAME + folder_suffix
...
class Agent():
    def __init__(self, num_actions):
        ...
        self.build_network = self.build
        self.train_network = self.train
        ...
        self.a_A, self.y_A, self.loss_A, self.grads_update_A = self.build_training_op(q_network_weights, 'a')
        self.a_B, self.y_B, self.loss_B, self.grads_update_B = self.build_training_op(target_network_weights, 'b')
    ...
    def build(self):
        ...
        model.add(Reshape( (STATE_LENGTH*FRAME_WIDTH*FRAME_HEIGHT,), input_shape=(STATE_LENGTH, FRAME_WIDTH, FRAME_HEIGHT) ) )
    ...
    def build_training_op(self, q_network_weights, network_type='a'):
        ...
        if network_type == 'a':
            ...
            q_value = tf.reduce_sum(tf.multiply(self.target_q_values, a_one_hot), reduction_indices=1)
        #q_value_target = tf.reduce_sum(tf.multiply(self.target_q_values, a_one_hot), reduction_indices=1)

        #q_value = 0.5 * tf.add(q_value_online, q_value_target)
    ...
    def train(self):
        ...
        random_num = np.random.binomial(1, 0.5)
        if random_num == 1:
            ...
            q_values_batch = self.q_values.eval(feed_dict={self.s: np.float32(np.array(next_state_batch) / 255.0)})
            y_batch = reward_batch + (1 - terminal_batch) * GAMMA * np.max(q_values_batch, axis=1)


        if random_num == 0:
            loss, _ = self.sess.run([self.loss_A, self.grads_update_A], feed_dict={
                self.s: np.float32(np.array(state_batch) / 255.0),
                self.a_A: action_batch,
                self.y_A: y_batch
            })

        else:
            loss, _ = self.sess.run([self.loss_B, self.grads_update_B], feed_dict={
                self.st: np.float32(np.array(state_batch) / 255.0),
                self.a_B: action_batch,
                self.y_B: y_batch
            })
    ...
    def get_action_at_test(self, state):
        if random.random() <= 0.05:
            ...
            a_q_values = self.q_values.eval(feed_dict={self.s: [np.float32(state / 255.0)]})
            b_q_values = self.target_q_values.eval(feed_dict={self.s: [np.float32(state / 255.0)]})
            action = np.argmax(0.5 * tf.add(a_q_values, b_q_values))

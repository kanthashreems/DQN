...
from keras.layers import Convolution2D, Flatten, Dense, Embedding, Reshape
from keras import backend as K
K.set_image_dim_ordering('th')

ENV_NAME = 'FrozenLake-v0'  # Environment name
...
class Agent():
    ...
    def build_network(self):
        ...
        model.add(Embedding(16, self.num_actions, input_shape=(16,)))
        model.add(Reshape((self.num_actions,)))

        s = tf.placeholder(tf.float32, [None, 16])
        q_values = model(s)
    ...
    def get_initial_state(self, observation, last_observation):
        ...
        state = [processed_observation for _ in range(1)]
        return np.stack(state, axis=0)

    def run(self, state, action, reward, terminal, observation):
        # next_state = np.append(state[1:, :, :], observation, axis=0)
        next_state = tf.one_hot(state, 16,1.0,0.0).eval()

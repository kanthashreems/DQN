...
parser.add_argument("--exp_replay", type=str, default="yes", help="Experience Replay true or false")
parser.add_argument("--target_fixing", type=str, default="yes", help="Target Fixing true or false")
...
exp_replay = opts.exp_replay
target_fixing = opts.target_fixing
...
LOAD_NETWORK = False
TRAIN = True
FOLDER_TAG = 'linear_' + exp_replay + '-' + target_fixing + '-' + ENV_NAME + folder_suffix
...
class ReplayMemory():
    def __init__(self, size, use_replay=True):
    ...
    def get_minibatch(self, batch_size):
        if self.use_replay == True:
            minibatch = random.sample(self.replay_memory, batch_size)
        else:
            minibatch = [self.replay_memory[i] for i in range(len(self.replay_memory)-32, len(self.replay_memory))]  
        return minibatch
...
class Agent():
    def __init__(self, num_actions):
        ...
        self.replay_memory_flag = exp_replay

        if self.replay_memory_flag == 'yes':
            self.replay_memory = ReplayMemory(NUM_REPLAY_MEMORY, use_replay=True)

        else:
            self.replay_memory = ReplayMemory(NUM_REPLAY_MEMORY, use_replay=False)
    ...
    def run(self, state, action, reward, terminal, observation):
        ...
        if self.t >= INITIAL_REPLAY_SIZE:
            ...
            if self.t % TARGET_UPDATE_INTERVAL == 0 and self.target_fixing == 'yes':
                self.sess.run(self.update_target_network)
